...
// FALLBACK ONLY as of the precinct-data integration: src/lib/midterms/precinctAnchor.ts
// now computes the real figure straight from public/data/precincts/manifest.json
// (the same 163,925-precinct NYT/MEDSL set the Results page's US precinct map
// uses), so these hand-typed numbers are only reached for a state the precinct
// set doesn't cover (currently just AK) or before that fetch resolves.
export const STATE_PVI_2024_FALLBACK: Record<string, number> = {
