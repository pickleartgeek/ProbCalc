 ...
 */
// Relative to BASE_URL (Vite's `base` config), not the domain root — an
// absolute leading-slash path like '/data/...' 404s on any deploy that
// isn't served from the root (e.g. a GitHub Pages project page at
// /<repo>/), because the browser resolves it against the origin instead of
// the app's actual base path.
const DEFAULT_URL = `${import.meta.env.BASE_URL}data/germany-grid.json`;

export async function loadGermanyGrid(url = DEFAULT_URL): Promise<{ features: RawFeature[]; bbox: [number, number, number, number] }> {
